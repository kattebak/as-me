---
name: juicebox
description: Install and set up the `juicebot` GitHub App wrapper. Replaces full-privilege OAuth tokens with manifest-scoped App credentials for `git` and `gh`. Use when the user asks to install or configure `juicebot`, wants scoped GitHub credentials on this machine, or refers to 🧃 / on-behalf-of / bot mode (the agent-attribution wrapper around `gh`).
---

End state: `gh` and `git push` use a manifest-scoped App token instead of a full-privilege OAuth token. `juicebot status` is the source of truth for where the user is in setup — run it whenever you're unsure which step to drive next.

## 1. Install the CLI

```sh
command -v juicebot >/dev/null || curl -fsSL https://raw.githubusercontent.com/kattebak/juicebox/main/install.sh | bash
```
...
Run `juicebot status` and parse the output. Map to the next section:
...
```sh
juicebot init [--org <org>] [--name <slug>] [--description <text>]
```
...
**Critical — do this immediately after the App is created, before §5:** open the App's settings page → "Identifying and authorizing users" → toggle **Enable Device Flow** on. The manifest cannot set this flag; without it `juicebot login` aborts with `device_flow_disabled`. The exact URL is whatever `juicebot init` prints as `app created: …` (also visible later via `juicebot status` as `html url`) — don't guess the slug; GitHub appends a suffix when the name is taken.

**Visibility — only matters if the user wants single-tenant lockdown:** the wizard defaults to public, meaning the App can be installed on any account (yours or anyone else's), each installation isolated to the installer's chosen repos and bounded by the manifest permission ceiling. If the user wants single-tenant lockdown (only the owning account can install), they should pick **Private** in the wizard at init time, OR pass `--private` to the CLI. Private → public is reversible later under *App settings → Advanced → Make public*; the reverse is one-way once anyone else has installed.

## 4. `juicebot install`

```sh
juicebot install [--org <org>]
```
...
- Shell rc: append `eval "$(juicebot env)"` to `~/.bashrc` or `~/.zshrc` (match `$SHELL`) only if not already present.
...
```sh
eval "$(juicebot env)" && gh api user --jq '.login'
```
...
`juicebot gh <gh-args…>` runs `gh` with an App installation token instead of the user's token, and prepends `🧃 created on behalf of @<login>` to PR/issue bodies. It's the agent-attribution mode: anyone reading the PR can see at a glance that an agent (not the human) wrote it.
...
**Routing:** bot mode reads the current repo's `origin` remote, takes the owner from it, and looks up the installation in state. If the owner has no installation, the user needs `juicebot install --org <owner>` first.

Example:

```sh
juicebot gh pr create --title "fix flake in api tests" --body "small retry tweak"
# body becomes:
